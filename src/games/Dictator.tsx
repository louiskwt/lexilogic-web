        ...
        <div className="flex flex-col items-center justify-center">
          <div className="text-4xl font-bold mb-4 flex justify-center">
            {currentWord.word.split("").map((_, index) => (
              <input key={index} ref={inputRefsArray[index]} type="text" value={userInput[index] || ""} maxLength={1} onClick={() => setCurrentIndex(index)} onChange={(e) => handleUserInput(e, index)} className={`bg-transparent border-b-2 mr-2 md:mr-5 border-gray-400 focus:border-lime-600 focus:outline-none w-8 md:w-12 md:h-12 text-center text-4xl ${userInput[index] === currentWord.word[index].toLowerCase() ? "text-green-500" : "text-white"}`} />
            ))}
          </div>
